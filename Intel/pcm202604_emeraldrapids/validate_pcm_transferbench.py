...
DEFAULT_TRANSFERBENCH = Path(
    os.environ.get("TRANSFERBENCH_BIN", "TransferBenchCuda")
)
DEFAULT_CUDA_LIB = (
    Path(value) if (value := os.environ.get("CUDA_LIB_DIR")) else None
)
ALL_CASES = (
    "combined_h2d",
    "combined_d2h",
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--cuda-lib",
        type=Path,
        default=DEFAULT_CUDA_LIB,
        help="directory prepended to LD_LIBRARY_PATH (default: $CUDA_LIB_DIR, if set)",
    )
...
def transferbench_environment(args: argparse.Namespace) -> dict[str, str]:
    ...
    if args.cuda_lib is not None:
        ...
        environment["LD_LIBRARY_PATH"] = str(args.cuda_lib) + (
...
def case_spec(args: argparse.Namespace, case: str) -> tuple[str, list[str], str]:
    ...
    if case.startswith("combined_"):
        ...
        # UPI counters work through Linux perf on Emerald Rapids. Avoid
        # elevating merely because optional MCFG-backed TPMI metrics are not
        # available to the calling user.
        extras = ["--no-cores", "--no-sudo"]
...
def preflight(args: argparse.Namespace, environment: dict[str, str]) -> None:
    ...
    if args.cuda_lib is not None and not args.cuda_lib.expanduser().is_dir():
        raise RuntimeError(f"CUDA library directory does not exist: {args.cuda_lib}")
    ...
    if "0 GPU device(s)" in result.stdout and any(
        ...
        raise RuntimeError(
            ...
            "access to the NVIDIA device nodes."
        )
    ...
    try:
        ...
        manifest: dict[str, object] = {
            ...
            "arguments": vars(args) | {
                ...
                "cuda_lib": str(args.cuda_lib) if args.cuda_lib else None,
